...
function bindFileConverter(){const f=document.getElementById('convertFile'),fmtEl=document.getElementById('convertFormat'),btn=document.getElementById('convertBtn'),st=document.getElementById('convertStatus'),prog=document.getElementById('convertProgress'),res=document.getElementById('convertResult');const map={mp4:[['avi','AVI'],['webm','WEBM'],['mov','MOV']],avi:[['mp4','MP4'],['webm','WEBM']],mov:[['mp4','MP4'],['avi','AVI'],['webm','WEBM']],mkv:[['mp4','MP4'],['webm','WEBM']],webm:[['mp4','MP4'],['avi','AVI']],jpg:[['png','PNG'],['webp','WEBP'],['pdf','PDF']],jpeg:[['png','PNG'],['webp','WEBP'],['pdf','PDF']],png:[['jpg','JPG'],['webp','WEBP'],['pdf','PDF']],webp:[['jpg','JPG'],['png','PNG'],['pdf','PDF']],bmp:[['jpg','JPG'],['png','PNG'],['webp','WEBP']],pdf:[['jpg','JPG (ZIP se houver várias páginas)'],['png','PNG (ZIP se houver várias páginas)'],['dxf','DXF (CAD vetorial)'],['dwg','DWG (CAD)']]};f?.addEventListener('change',()=>{const file=f.files?.[0];if(!file)return;const ext=(file.name.split('.').pop()||'').toLowerCase(),opts=map[ext]||[];fmtEl.innerHTML=opts.length?opts.map(x=>'<option value="'+x[0]+'">'+x[1]+'</option>').join(''):'<option value="">Formato não suportado</option>';btn.disabled=!opts.length;st.innerHTML='Arquivo: <strong>'+esc(file.name)+'</strong> · '+num(file.size/1024/1024)+' MB';});btn?.addEventListener('click',async()=>{const file=f?.files?.[0],out=fmtEl?.value;if(!file||!out)return;btn.disabled=true;prog.style.display='block';st.textContent='Enviando e convertendo...';try{const fd=new FormData();fd.append('file',file);fd.append('output_format',out);const r=await fetch('/api/files/convert',{method:'POST',body:fd});if(!r.ok){let d={};try{d=await r.json()}catch{}throw new Error(d.detail||'Não foi possível converter o arquivo.');}const blob=await r.blob(),cd=r.headers.get('content-disposition')||'',m=cd.match(/filename="?([^"]+)"?/i),name=m?m[1]:'arquivo-convertido.'+out,url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=name;a.textContent='⬇️ Baixar arquivo convertido';a.className='btn primary';res.innerHTML='<div class="result-box"><div class="result-label">Conversão concluída</div><div class="result-main">✓</div><p>'+esc(name)+'</p></div>';res.querySelector('.result-box').appendChild(a);st.textContent='✅ Conversão concluída.';a.click();setTimeout(()=>URL.revokeObjectURL(url),10000);}catch(e){st.innerHTML='⚠️ '+esc(e.message);}finally{prog.style.display='none';btn.disabled=false;}});document.getElementById('convertResetBtn')?.addEventListener('click',()=>{f.value='';fmtEl.innerHTML='<option value="">Selecione o arquivo primeiro</option>';btn.disabled=true;st.textContent='Nenhum arquivo selecionado.';});}
function bind(){
